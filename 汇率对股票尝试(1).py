...
data = pd.read_excel(r'C:\Users\PKF\Desktop\实习\ycq\6.xlsx')


# Define the independent variables (features) and the dependent variable (target)
X = data[['BIS指数', 'UsDCNY', 'CFETS指数', 'SDR货币指数']]
y = data['000300.SH']
...
doc = Document()
doc.add_heading('第五章 汇率指数对上证指数影响的机器学习模型分析', level=0)
...
def evaluate_model(model, model_name, model_intro):
    ...
    plt.plot(range(len(y_train_pred)), y_train_pred, label='预测值', color='red', linestyle='--')
    plt.title(f'{model_name} - 训练集预测值与实际值对比')
    plt.xlabel('样本索引')
    plt.ylabel('上证指数(000300.SH)')
    ...
    plt.plot(range(len(y_test_pred)), y_test_pred, label='预测值', color='red', linestyle='--')
    ...
    if model_name == '多元线性回归模型':
        conclusion = f"""该多元线性回归模型在训练集上的R2为{train_r2:.4f}，在测试集上的R2为{test_r2:.4f}。模型表现出一定的预测能力，但R2值相对较低，说明线性模型可能无法完全捕捉汇率指数与上证指数之间的复杂关系。测试集RMSE为{test_rmse:.4f}，表明预测值与实际值之间存在一定偏差。这一结果表明，汇率指数与上证指数之间可能存在非线性关系，需要更复杂的模型来进行建模。"""
        ...
        conclusion = f"""XGBoost模型在训练集上的R2为{train_r2:.4f}，在测试集上的R2为{test_r2:.4f}。该模型展现出极强的拟合能力，测试集RMSE为{test_rmse:.4f}，明显优于线性模型和神经网络。训练集和测试集的性能差异表明模型具有良好的泛化能力。XGBoost能够自动处理特征间的交互作用，这对于捕捉汇率指数与上证指数之间的复杂关系非常有效。该结果强烈表明，汇率指数对上证指数具有显著的预测价值。"""
    elif model_name == '随机森林模型':
        conclusion = f"""随机森林模型在训练集上的R2为{train_r2:.4f}，在测试集上的R2为{test_r2:.4f}。该模型表现出较强的预测能力，测试集RMSE为{test_rmse:.4f}。随机森林通过集成多棵决策树的预测结果，有效减少了过拟合风险，提高了模型的泛化能力。这一结果表明，汇率指数与上证指数之间存在可被捕捉的关系模式，且这种关系可能是非线性的。"""
    elif model_name == '梯度提升树模型':
        conclusion = f"""梯度提升树模型在训练集上的R2为{train_r2:.4f}，在测试集上的R2为{test_r2:.4f}。该模型展现出较好的预测性能，测试集RMSE为{test_rmse:.4f}。梯度提升树通过顺序构建决策树并纠正前面树的预测误差，能够有效捕捉汇率指数与上证指数之间的关系。模型的表现进一步证实了汇率指数对上证指数具有预测价值。"""
    elif model_name == '长短期记忆网络模型':
        conclusion = f"""长短期记忆网络(LSTM)模型在训练集上的R2为{train_r2:.4f}，在测试集上的R2为{test_r2:.4f}。该模型在处理时间序列数据方面表现出色，能够捕捉数据中的长期依赖关系。测试集RMSE为{test_rmse:.4f}，表明模型在预测上证指数方面具有一定的预测能力。与加楚懿(2025)和于孝建(2024)的研究发现相符，LSTM在金融时间序列分析中的应用价值得到了证实。"""
...
多元线性回归是一种基础的统计学习方法，用于建立因变量与多个自变量之间的线性关系。该模型假设因变量与自变量之间存在线性关系，通过最小化预测值与实际值之间的均方误差来确定最优参数。在本研究中，我们使用多元线性回归来分析汇率指数对上证指数的影响。"""

xgb_intro = """5.2.4 XGBoost模型
XGBoost(eXtreme Gradient Boosting)是一种高效的梯度提升树算法，通过集成多个决策树来提高预测性能。该算法采用了正则化技术来控制模型复杂度，并使用二阶导数来优化损失函数，具有高效、灵活和可扩展的特点。在本研究中，我们利用XGBoost来分析汇率指数对上证指数的影响，该模型能够自动处理特征之间的交互作用。"""

rf_intro = """5.2.5 随机森林模型
随机森林是一种集成学习方法，通过构建多个决策树并将它们的预测结果进行组合来提高模型性能。每棵树在随机选择的数据子集上训练，并在每个节点使用随机选择的特征子集进行分裂，从而减少过拟合风险。随机森林模型具有较强的鲁棒性和解释性，能够评估特征重要性。在本研究中，我们应用随机森林来分析汇率指数对上证指数的影响程度。"""

gb_intro = """5.2.6 梯度提升树模型
梯度提升树是一种顺序构建决策树的集成方法，每棵新树都试图纠正前面树的预测误差。通过梯度下降算法优化损失函数，该模型能够逐步提高预测精度。梯度提升树对异常值不敏感，能够自动处理特征间的交互作用，并能处理各种类型的数据。在本研究中，我们使用梯度提升树来建立汇率指数与上证指数之间的预测模型。"""
...
def perform_cross_validation(model, model_name):
    ...
    light_purple = np.array([221/255, 204/255, 239/255])
    dark_purple = np.array([147/255, 111/255, 184/255])
    
    # Normalize R2 values for color mapping
    if max(r2_scores) != min(r2_scores):
        norm_r2 = [(r2 - min(r2_scores)) / (max(r2_scores) - min(r2_scores)) 
                  for r2 in r2_scores]
    else:
        norm_r2 = [0.5] * len(r2_scores)
    ...
    # Add interpretation
    interpretation = f"""交叉验证结果表明，{model_name}在不同的数据子集上表现相对一致，平均R2为{mean_r2:.4f}，标准差为{std_r2:.4f}。平均RMSE为{mean_rmse:.4f}，标准差为{std_rmse:.4f}。这些结果表明{model_name}在预测上证指数方面具有一定的稳定性和泛化能力。"""
...
print("\n开始训练LSTM模型...")
lstm_results = evaluate_model(LSTMRegressor(units=32, epochs=50, batch_size=32, random_state=42), 
                             '长短期记忆网络模型', lstm_intro)
all_results.append(lstm_results)

print("\n开始训练ELM模型...")
elm_model = ELMRegressor(
    n_hidden=200,           # Reduced hidden neurons
...
def plot_feature_importance(model, model_name, is_cv=False):
    ...
    interpretation = f"""特征重要性分析表明，在{cv_text}XGBoost模型中，对上证指数预测影响最大的三个特征是{', '.join(top_features)}，
这表明这些股票市场指标与上证指数之间存在较强的关联。相比之下，{', '.join(bottom_features)}的影响相对较小。
这一结果为理解股票市场指标对上证指数的影响机制提供了重要参考。"""
    doc.add_paragraph(interpretation)
...
models = [
    ...
    (xgb.XGBRegressor(random_state=42, n_estimators=100), 'XGBoost模型'),  # Reduced number of trees
    (RandomForestRegressor(random_state=42, n_estimators=100), '随机森林模型'),  # Reduced number of trees
    (GradientBoostingRegressor(random_state=42, n_estimators=100), '梯度提升树模型'),  # Reduced number of trees
    (LSTMRegressor(units=32, epochs=50, batch_size=32, random_state=42), '长短期记忆网络模型'),  # Reduced units and epochs
    (ELMRegressor(
        n_hidden=200,  # Reduced hidden neurons
        activation='tanh',
        alpha=0.01,
...
综合所有七种模型的交叉验证结果，我们可以更加全面地评估不同机器学习方法在预测上证指数方面的表现。交叉验证结果的一致性增强了我们对模型选择的信心，为后续研究和实际应用提供了可靠的依据。""")
...
lstm_norm_results = evaluate_normalized_model(
    LSTMRegressor(units=32, epochs=50, batch_size=32, random_state=42), 
    '长短期记忆网络模型'
)
...
elm_norm_results = evaluate_normalized_model(
    ELMRegressor(n_hidden=200, activation='tanh', alpha=0.01, random_state=42),
    '极限学习机模型'
)
...
def plot_normalized_feature_importance(model):
    ...
    # Use a simple blue color
    bars = plt.bar(feature_importance['Feature'], feature_importance['Importance'], color='blue')
    ...
    interpretation = f"""标准化数据的特征重要性分析表明，在XGBoost模型中，对上证指数预测影响最大的三个特征是{', '.join(top_features)}，
这表明这些股票市场指标与上证指数之间存在较强的关联。相比之下，{', '.join(bottom_features)}的影响相对较小。
...
def perform_normalized_cross_validation(model, model_name):
    ...
    # Add interpretation
    interpretation = f"""标准化数据的交叉验证结果表明，{model_name}在不同的标准化数据子集上表现相对一致，平均R2为{mean_r2:.4f}，标准差为{std_r2:.4f}。平均RMSE为{mean_rmse:.4f}，标准差为{std_rmse:.4f}。这些结果表明{model_name}在预测标准化上证指数方面具有一定的稳定性和泛化能力。"""
    doc.add_paragraph(interpretation)
...
# Add results for each model
common_models = [model_name for model_name in original_cv_dict.keys() if model_name in normalized_cv_dict]
for i, model_name in enumerate(common_models):
    row_cells = comparison_table.rows[i+1].cells
    row_cells[0].text = model_name
...
doc.add_paragraph(f"""在标准化数据的交叉验证结果中，{best_norm_cv_model_name}表现最佳，平均R²为{max(normalized_cv_results, key=lambda x: x[1]['mean_r2'])[1]['mean_r2']:.4f}。以下分析该模型的特征重要性，以深入了解各股票市场指标对上证指数的影响程度。""")
...
if best_norm_cv_model_name == 'XGBoost模型':
    ...
    interpretation = f"""特征重要性分析表明，在最佳标准化模型({best_norm_cv_model_name})中，对上证指数预测影响最大的三个特征是{', '.join(top_features)}，
这表明这些汇率指标与上证指数之间存在较强的关联。相比之下，{', '.join(bottom_features)}的影响相对较小。

这些高重要性特征可能代表了汇率市场与股票市场之间关联的关键因素，为理解两个市场之间的相互作用机制提供了重要线索。在实际应用中，可以重点关注这些高重要性特征的变化，以更好地预测上证指数的走势。"""
    doc.add_paragraph(interpretation)
...
doc.add_paragraph("""本章运用多种机器学习模型对汇率指数与上证指数之间的关系进行了系统分析，通过模型训练、评估、交叉验证和数据标准化等多个环节，得出以下主要结论：

1. 汇率与股票市场的关联性：多种机器学习模型的预测结果均表明，汇率指数与上证指数之间存在显著的关联模式。这种关联不仅仅是简单的线性关系，更多地表现为复杂的非线性关系，这解释了为何非线性模型（如XGBoost和LSTM）能够取得更好的预测效果。

2. 模型性能比较：在所有评估的模型中，XGBoost模型和长短期记忆网络(LSTM)模型展现出最优的预测性能，其R²值和RMSE指标均优于其他模型。这一结果表明，集成学习方法和深度学习方法在捕捉金融时间序列数据中的复杂模式方面具有显著优势。

3. 特征重要性洞察：通过对XGBoost等模型的特征重要性分析发现，某些特定的汇率指标对上证指数的预测贡献度明显高于其他指标。这些高重要性特征主要包括BIS指数和CFETS指数，表明这些因素可能是连接汇率市场与股票市场的关键桥梁。
...
综上所述，本章研究不仅验证了汇率指数与上证指数之间存在稳健的预测关系，还通过多种机器学习模型的对比分析，揭示了这种关系的复杂性和非线性特征。这些发现为理解全球金融市场的相互作用机制提供了新的视角，同时也为投资决策、风险管理和宏观经济政策制定提供了数据驱动的参考依据。未来研究可以进一步探索这些关联背后的经济机制，以及如何将这些模型预测结果整合到实际的金融决策系统中。""")

# After evaluating all models, add a summary section comparing test metrics
doc.add_heading('5.2.9 模型性能汇总比较', level=2)
doc.add_paragraph("""本节对所有模型在测试集上的性能指标进行汇总比较，以便直观地评估不同模型的预测能力。通过比较R²、RMSE、MAE和MAPE等指标，我们可以全面了解各模型的优缺点，为后续研究和实际应用提供参考。""")
...
for i, result in enumerate(sorted_results):
    ...
    row_cells[1].text = f"{result['test_r2']:.4f}"
    row_cells[2].text = f"{result['test_mse']:.4f}"
    row_cells[3].text = f"{result['test_rmse']:.4f}"
    row_cells[4].text = f"{result['test_mae']:.4f}"
    row_cells[5].text = f"{result['test_mape']:.4f}"

# Create bar chart comparing test R² values
plt.figure(figsize=(12, 6))

# Get model names and R² values
model_names = [result['model_name'] for result in sorted_results]
r2_values = [result['test_r2'] for result in sorted_results]

# Create color gradient from light to dark blue
light_blue = np.array([173/255, 216/255, 230/255])  # Light blue
dark_blue = np.array([0/255, 0/255, 139/255])       # Dark blue

# Normalize R² values for color mapping
if max(r2_values) != min(r2_values):
    norm_r2 = [(r2 - min(r2_values)) / (max(r2_values) - min(r2_values)) 
              for r2 in r2_values]
else:
    norm_r2 = [0.5] * len(r2_values)

# Generate colors for each bar
r2_colors = [light_blue + t * (dark_blue - light_blue) for t in norm_r2]

# Create bar chart
bars = plt.bar(model_names, r2_values, color=r2_colors)
plt.title('各模型测试集 R² 对比')
plt.xlabel('模型')
plt.ylabel('R²')
plt.xticks(rotation=45, ha='right')
...
# Save the figure
r2_comparison_path = f'{output_dir}\\model_r2_comparison.png'
plt.savefig(r2_comparison_path)
plt.close()

# Add the figure to the document
doc.add_paragraph().add_run().add_picture(r2_comparison_path, width=Inches(6))

# Create bar chart comparing test RMSE values
plt.figure(figsize=(12, 6))

# Get RMSE values
rmse_values = [result['test_rmse'] for result in sorted_results]

# For RMSE, lower is better, so we invert the color mapping
if max(rmse_values) != min(rmse_values):
    norm_rmse = [1 - (rmse - min(rmse_values)) / (max(rmse_values) - min(rmse_values)) 
                for rmse in rmse_values]
else:
    norm_rmse = [0.5] * len(rmse_values)

# Generate colors for each bar
rmse_colors = [light_blue + t * (dark_blue - light_blue) for t in norm_rmse]

# Create bar chart
bars = plt.bar(model_names, rmse_values, color=rmse_colors)
plt.title('各模型测试集 RMSE 对比')
plt.xlabel('模型')
plt.ylabel('RMSE')
plt.xticks(rotation=45, ha='right')
plt.grid(axis='y', linestyle='--', alpha=0.7)
...
# Save the figure
rmse_comparison_path = f'{output_dir}\\model_rmse_comparison.png'
plt.savefig(rmse_comparison_path)
plt.close()

# Add the figure to the document
doc.add_paragraph().add_run().add_picture(rmse_comparison_path, width=Inches(6))

# Add summary interpretation
best_model = sorted_results[0]['model_name']
best_r2 = sorted_results[0]['test_r2']
best_rmse = sorted_results[0]['test_rmse']

doc.add_paragraph(f"""模型性能汇总比较表明，在所有评估的模型中，{best_model}表现最佳，测试集R²达到{best_r2:.4f}，RMSE为{best_rmse:.4f}。这表明该模型在捕捉汇率指数与上证指数之间的关系方面具有较强的能力。

从整体来看，基于树的集成模型（如XGBoost、随机森林和梯度提升树）以及极限学习机模型表现较好，这可能是因为它们能够有效捕捉数据中的非线性关系。相比之下，线性模型的表现相对较弱，这进一步证实了汇率指数与上证指数之间可能存在复杂的非线性关系。

这些结果为我们选择合适的模型进行汇率指数对上证指数影响的预测和分析提供了重要依据。在实际应用中，可以根据具体需求（如预测精度、模型解释性、计算效率等）选择合适的模型。""")

# Add feature importance analysis for ELM model
doc.add_heading('5.2.10 极限学习机模型特征重要性分析', level=2)
doc.add_paragraph("""由于极限学习机模型在测试集上表现出色，本节对该模型的特征重要性进行分析，以了解不同汇率指数对上证指数的影响程度。由于ELM模型本身不直接提供特征重要性，我们采用排列重要性(Permutation Importance)方法来评估特征的重要性。""")

# Function to calculate permutation importance for ELM model
def calculate_elm_feature_importance(model, X, y, n_repeats=10, random_state=42):
    """
    ...
    model : fitted model object
        The model for which to calculate feature importance
    X : DataFrame
        Feature data
    y : Series
        Target data
...
# Train ELM model on full dataset
elm_model_full = ELMRegressor(
    n_hidden=500,
    activation='tanh',
    alpha=0.01,
    random_state=42
)
elm_model_full.fit(X, y)

# Calculate feature importance
elm_importance = calculate_elm_feature_importance(elm_model_full, X, y)

# Plot feature importance using the same style as XGBoost
plt.figure(figsize=(10, 6))
...
# Create color gradient from light to dark purple (same as XGBoost)
light_purple = np.array([221/255, 204/255, 239/255])  # Light purple RGB(221, 204, 239)
dark_purple = np.array([147/255, 111/255, 184/255])   # Dark purple RGB(147, 111, 184)

# Normalize importance values for color mapping
if max(elm_importance['Importance']) != min(elm_importance['Importance']):
    norm_importance = [(imp - min(elm_importance['Importance'])) / 
                      (max(elm_importance['Importance']) - min(elm_importance['Importance'])) 
                      for imp in elm_importance['Importance']]
else:
    norm_importance = [0.5] * len(elm_importance)
...
bars = plt.bar(
    elm_importance['Feature'], 
    elm_importance['Importance'],
    color=importance_colors
)

# Add title and labels
plt.title('极限学习机模型 - 特征重要性')
plt.xlabel('特征')
plt.ylabel('重要性')
plt.xticks(rotation=45, ha='right')
...
# Save the figure
elm_importance_path = f'{output_dir}\\极限学习机模型_feature_importance.png'
plt.savefig(elm_importance_path)
plt.close()

# Add the figure to the document
doc.add_heading('极限学习机模型 - 特征重要性分析', level=3)
doc.add_paragraph().add_run().add_picture(elm_importance_path, width=Inches(6))

# Add interpretation
top_features = elm_importance.head(3)['Feature'].tolist()
bottom_features = elm_importance.tail(3)['Feature'].tolist()

interpretation = f"""特征重要性分析表明，在极限学习机模型中，对上证指数预测影响最大的三个特征是{', '.join(top_features)}，
这表明这些汇率指数与上证指数之间存在较强的关联。相比之下，{', '.join(bottom_features)}的影响相对较小。
这一结果为理解汇率指数对上证指数的影响机制提供了重要参考。"""
doc.add_paragraph(interpretation)

# Save the Word document
doc.save(r'C:\Users\PKF\Desktop\实习\ycq\第五章_汇率指数对上证指数影响的机器学习模型分析.docx')
